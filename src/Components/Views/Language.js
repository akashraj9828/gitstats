...
const Language = (props) => {
    ...
    // LIST OF LANGUAGES
    const language_dom_list = datasource.map((language) => language.value===0? "" : <li key={language.key}> <span style={{color:language_color_data[language.key]}}> {language.key}  </span>: {language.value} {suffix}</li>)
    return (
        <div>
            <ol>
                {language_dom_list}
