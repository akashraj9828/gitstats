...
let { toReadableBytes, sortObject, insertAtIndex } = util;
...
async function repoBarGraphCalculation(repoInfo) {
	...
	var toggle = false;
	let color_pallet = ["#f47560", "#e8c1a0", "#f1e15b", "#e8a838", "#61cdbb", "#97e3d5"];
	const { sorted_by_commits, sorted_by_popularity } = repoInfo;
	try {
		// eslint-disable-next-line no-unused-vars
...
async function commitGraphDataDayWise(commitHistoryData) {
	...
	for (let i = 0; i < 7; i++) {
		...
		week_graph_data.push({
			...
			commitColor: `rgb(30,${parseInt(((week_commit_activity[week_day] * 0.7) / max_count) * 256)},30)`,
